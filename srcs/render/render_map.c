...
/*   Updated: 2020/12/04 00:19:51 by ynakamot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_map_floor(t_data *map, int x, int y)
{
	int i;
	int j;
	...
	while (i < 20)
	{
		...
		my_mlx_pixel_put(map, (x + 1) * 20 - 1, y * 20 + i, 0x00B007);
		my_mlx_pixel_put(map, x * 20 + i, (y + 1) * 20 - 1, 0x00B007);
		i++;
	}
}
...
void	render_map_wall(t_data *map, int x, int y)
{
	...
	while (i < 20)
	{
		my_mlx_pixel_put(map, x * 20, y * 20 + i, 0x012217);
		my_mlx_pixel_put(map, x * 20 + i, y * 20, 0x012217);
		my_mlx_pixel_put(map, (x + 1) * 20 - 1, y * 20 + i, 0x012217);
		my_mlx_pixel_put(map, x * 20 + i, (y + 1) * 20 - 1, 0x012217);
		i++;
	}
}
